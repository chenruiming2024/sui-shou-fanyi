...
- **Tray + global shortcut**: tray menu lives in `src-tauri/src/lib.rs` setup; the global shortcut is registered from the frontend (`SettingsPanel` on mount, value from localStorage, default `Alt+T`) via the `window::register_shortcut` command, which does `unregister_all` first.
